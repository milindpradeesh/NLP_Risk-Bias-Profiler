...
def main():
    ...
    print("\nWelcome to NLP Risk Profiling and Bias Identifier Tool\n")
    ...
    for i, question in enumerate(questions, start=1):
        print(f"\nQuestion {i}: {question}")  # Display question during runtime
        user_response = input("Your response: ").strip()
        
        while not user_response:
            print("Invalid response. Please enter a valid sentence.")
            user_response = input("Your response: ").strip()
    ...
    print(f"Overall Risk Profile: {highest_risk_category}")
    print(f"Dominant Behavioral Bias: {highest_bias_category}")
